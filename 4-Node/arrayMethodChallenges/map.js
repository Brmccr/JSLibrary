...
//console log first and last names from array 


let presidentsNames = presidents.map(presidents => presidents.first + ' ' + presidents.last);
console.log(presidentsNames);

let = fullname = presidents.map(president => `${president.first} ${presdident.last}`) 




function getFullName(presidents) {
...
var presidentsName= presidents.map(president => presidents[0].first); {
return ( <FilterResults
    key = {presidents.first}
    space={presidents.last}
...
console.log(presidentsName);

var presidentName = presidents.map.get